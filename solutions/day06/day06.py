...
def pt1(input: str):
    SIM_DAYS = 80
    ages = np.array(input.split(','), dtype=int)
    for _ in range(SIM_DAYS):
        ages -= 1

        ages = np.append(ages, [8] * np.sum(ages < 0)) # spawn new fish for fish that were age 0
        ages[ages < 0] = 6 # reset the ages of the fish that were age 0

    return len(ages)

PT2_SAMPLE_ANS = 26984457539

def pt2(input: str):
    SIM_DAYS = 256
    ages = np.array(input.split(','), dtype=int)
    agecounts = np.array([np.sum(ages == a) for a in range(0,9)], dtype=np.int64)

    for _ in range(SIM_DAYS):
        agecounts = np.roll(agecounts, -1)
        agecounts[6] += agecounts[8]
